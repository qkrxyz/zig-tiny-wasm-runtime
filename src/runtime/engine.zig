...
const ModuleInst = runtime_types.ModuleInst;
...
pub const Engine = struct {
    ...
    pub fn getModuleInstByName(self: *Self, module_name: []const u8) ?*ModuleInst {
        return self.mod_insts.get(module_name);
    }

    pub fn getValueFromGlobal(self: *Self, mod_inst: *ModuleInst, field_name: []const u8) ?Value {
    ...
    fn findExternValue(mod_inst: *ModuleInst, name: []const u8) ?ExternalValue {
    ...
    pub fn loadModuleFromPath(self: *Self, file_name: []const u8, module_name: ?[]const u8) !*ModuleInst {
    ...
    pub fn registerModule(self: *Self, mod_inst: *ModuleInst, module_name: []const u8) error{OutOfMemory}!void {
